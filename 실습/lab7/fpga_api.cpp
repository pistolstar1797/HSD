  ...
  for (int i = 0; i < num_output; i += m_size_)
  {
    for (int j = 0; j < num_input; j += v_size_)
    {
      ...
      for(int k = 0 ; k < block_row ; k++)
	      memcpy(mat + v_size_*k, large_mat + num_input*(i+k) + j, block_col*sizeof(float));
		...
		for(int j = 0 ; j < input_channel ; j++)
			for(int k = 0 ; k < conv_height ; k++)
				for(int l = 0 ; l < conv_width ; l++)
					new_weights[i][(j*conv_height + k)*conv_width + l] = cnn_weights[i][j][k][l];

	int row_filter = input_height - conv_height + 1;
	int col_filter = input_width - conv_width + 1;

	for(int i = 0 ; i < input_channel ; i++)
		for(int j = 0 ; j < conv_height ; j++)
			for(int k = 0 ; k < conv_width ; k++)
				for(int m = 0 ; m < row_filter ; m++)
					for(int n = 0 ; n < col_filter ; n++)
						new_inputs[(i*conv_height + j)*conv_width + k][m*col_filter + n] = inputs[i][j+m][k+n];
}
